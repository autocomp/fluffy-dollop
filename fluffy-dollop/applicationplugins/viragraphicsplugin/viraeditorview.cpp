...
void ViraEditorView::slotSetMarkPosition(QVariant var)
{
    ...
    _editObjectGeometry = (type > 0 ? regionbiz::RegionBizManager::instance()->getCurrentEntity() : 0);
...
//                CommonMessageNotifier::send( (uint)visualize_system::BusTags::EditModeFinish, QVariant(markId), QString("visualize_system"));
        ...
        else
        {
            ...
            foreach(QGraphicsItem* item, _items)
            {
                ...
                if(viraGraphicsItem)
                {
                    regionbiz::RegionBizManager::instance()->setCurrentEntity(viraGraphicsItem->getId());
                    return;
                }
            }
...
void ViraEditorView::mouseDoubleClickEvent(QMouseEvent *e)
{
    if(e->button() & Qt::LeftButton)
    {
        ...
        else
        {
            ...
            regionbiz::RegionBizManager::instance()->setCurrentEntity(_currFloor_id); //0);
            QGraphicsView::mouseReleaseEvent(e);
        }
    }
}
